...
class KineticsDataset(BaseDataset):
    ...
    def _load_video_paths(self, vis_root):
        return dict([(path[:-18].split('/')[-1], path) for path in glob.glob(os.path.join(vis_root, '*.mp4'))])

    def _load_annotations(self, ann_root):
        annotations = pd.read_csv(ann_root)
        return annotations
    ...
    def __getitem__(self, index):
        ...
        for _ in range(num_retries):
            ...
            video_id = sample_dict['youtube_id']
            text = sample_dict['label']
...
class KineticsInstructDataset(BaseDataset):
